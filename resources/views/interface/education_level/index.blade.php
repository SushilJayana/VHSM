...
<h2 class="mb-4">Education Level Management</h2>
<div class="container">
    <div class="row">
        <a class="btn btn-info" href="{{ route('education_level.create') }}">Add</a>
        <table class="table table-bordered" style="margin-top:20px">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$educationLevel->slug}}</td>
                    <td>{{$educationLevel->created_at}}</td>
                    <td>
                        <form method="POST" action="{{ route('education_level.destroy', $educationLevel->id) }}">
                            @csrf
                            @method('DELETE')

                            <button type="button" title="Edit" class="btn btn-rounded btn-info"
                                onclick="window.location='{{route('education_level.edit',$educationLevel->id)}}'">
